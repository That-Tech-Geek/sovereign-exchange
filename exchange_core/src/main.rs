...
fn main() {
    ...
    if let Some(core_ids) = core_affinity::get_core_ids() {
        ...
        println!("ℹ️ CPU affinity detection unavailable on this environment");
    }
    ...
    println!(
        ...
        warmup_duration,
        warmup_duration.as_micros() as f64 / 10_000.0
    );
    ...
    let cancel_accepted = engine
        .accept_order(&cancel_packet)
        .expect("cancel command must be accepted");
    engine.process_command(cancel_accepted.pool_index);
    println!(
        "✅ Typed cancellation command verified (Client Order ID: {}, Exchange Order ID: {})",
