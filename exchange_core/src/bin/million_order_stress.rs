...
fn main() {
    ...
    println!("pairs={} total_orders={} instrument={} price={} qty={}", PAIRS, PAIRS * 2, INSTRUMENT, PRICE, QTY);

    let init = Instant::now();
    let mut engine = MatchingEngine::new();
    println!("engine_init_ms={:.3}", init.elapsed().as_secs_f64() * 1_000.0);
    ...
    for i in 0..PAIRS {
        let accepted = engine
            .accept_order(&packet(PAIRS as u64 + i as u64 + 1, (i % 100_000) as u32 + 1, 1))
    ...
    assert_eq!(engine.pool.allocated_count, 0, "all orders must be fully cleared");
