...
/// Event storage starts small but grows with the number of fills produced by a
/// single command. This is a capacity hint, not a matching limit.
...
pub struct MatchingEngine {
    ...
    /// All fills generated by the most recently processed command.
    /// Capacity grows automatically; it is never used as a semantic limit.
    pub trades: Vec<Trade>,
    pub trade_count: usize,
    /// All events generated by the most recently processed command.
    /// Capacity grows automatically with the number of fills.
...
impl MatchingEngine {
    ...
    pub fn accept_command(&mut self, command: &OrderCommand) -> Result<AcceptedOrder, OrderAcceptError> {
        ...
        let pool_index = self.pool.allocate_from_command(
            command,
            exchange_order_id,
    ...
    pub fn process_order(&mut self, idx: u32) -> usize {
        ...
        match command_kind {
            ...
            x if x == CommandKind::Replace as u8 => {
                let target = ClientOrderId(
                    self.pool.data[idx as usize].replace_target_client_order_id,
                );
