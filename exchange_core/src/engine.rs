...
pub struct Trade {
    pub buyer: u32, pub seller: u32, pub price: u32, pub qty: u32, pub instrument_id: u16,
    pub buyer_exchange_order_id: u64, pub seller_exchange_order_id: u64,
    pub buyer_client_order_id: u64, pub seller_client_order_id: u64,
    pub buyer_sequence_number: u64, pub seller_sequence_number: u64, pub timestamp: u64,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OrderAcceptError {
    InvalidInstrument(u16), InvalidSide(u8),
    ...
    ExchangeOrderIdExhausted, SequenceNumberExhausted,
}
impl OrderAcceptError {
    pub const fn reason(self) -> CommandRejectReason { match self {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AcceptedOrder { pub pool_index: u32, pub exchange_order_id: ExchangeOrderId, pub sequence_number: SequenceNumber }
...
impl MatchingEngine {
    pub fn new() -> Self { Self {
        pool: OrderPool::new(), books: (0..MAX_INSTRUMENTS).map(|_| OrderBook::new()).collect(),
        instruments: InstrumentRegistry::from_sovereigns(SOVEREIGNS),
        trades: [Trade::default(); MAX_TRADES_PER_MATCH], trade_count: 0,
        last_events: [None; MAX_EVENTS_PER_COMMAND], event_count: 0,
        next_exchange_order_id: 1, next_sequence_number: SequenceNumber::FIRST.0,
    }}
    #[inline(always)] pub fn instrument(&self, id: u16) -> Option<&Instrument> { self.instruments.get(id) }
    #[inline(always)] pub fn book(&self, id: u16) -> Option<&OrderBook> { self.books.get(id as usize) }
    #[inline(always)] pub fn book_mut(&mut self, id: u16) -> Option<&mut OrderBook> { self.books.get_mut(id as usize) }
    ...
    pub fn accept_command(&mut self, command: &OrderCommand) -> Result<AcceptedOrder, OrderAcceptError> {
        ...
        match *command {
            OrderCommand::New(_) if book.contains_order(account_id, client_order_id) => return Err(OrderAcceptError::DuplicateClientOrderId { instrument_id, account_id, client_order_id }),
            OrderCommand::Cancel(_) if !book.contains_order(account_id, client_order_id) => return Err(OrderAcceptError::UnknownOrder { instrument_id, account_id, client_order_id }),
            OrderCommand::Replace(o) => {
                if !book.contains_order(account_id, o.target_client_order_id) { return Err(OrderAcceptError::UnknownOrder { instrument_id, account_id, client_order_id: o.target_client_order_id }); }
                if o.new_client_order_id != o.target_client_order_id && book.contains_order(account_id, o.new_client_order_id) { return Err(OrderAcceptError::DuplicateClientOrderId { instrument_id, account_id, client_order_id: o.new_client_order_id }); }
            }
            _ => {}
        }
        ...
        let exchange_order_id = match command {
            ...
            OrderCommand::New(_) | OrderCommand::Replace(_) => {
                ...
                self.next_exchange_order_id = next; ExchangeOrderId(id)
            }
        };
    ...
    #[inline(always)] pub fn process_command(&mut self, idx: u32) -> usize { self.process_order(idx) }

    #[inline(always)]
    pub fn process_order(&mut self, idx: u32) -> usize {
        self.trade_count = 0; self.event_count = 0; self.last_events.fill(None);
        ...
        match command_kind {
            x if x == CommandKind::Cancel as u8 => {
                if self.cancel_order(instrument_id, account_id, client_order_id) { self.push_event(ExchangeEvent::OrderCancelled { instrument_id, account_id, client_order_id, sequence_number }); }
                self.pool.deallocate(idx); return 0;
            }
        ...
        self.push_event(ExchangeEvent::OrderAccepted { instrument_id, account_id, client_order_id, exchange_order_id: ExchangeOrderId(self.pool.data[idx as usize].exchange_order_id), sequence_number });

        {
            ...
            if pool.data[idx as usize].remaining > 0 { book.insert_limit(idx, pool); }
            else { pool.deallocate(idx); }
        }
        ...
        for i in 0..self.trade_count {
            ...
            self.push_event(ExchangeEvent::Trade {
                ...
                buyer_exchange_order_id: ExchangeOrderId(t.buyer_exchange_order_id), seller_exchange_order_id: ExchangeOrderId(t.seller_exchange_order_id),
                buyer_client_order_id: ClientOrderId(t.buyer_client_order_id), seller_client_order_id: ClientOrderId(t.seller_client_order_id),
                buyer_sequence_number: SequenceNumber(t.buyer_sequence_number), seller_sequence_number: SequenceNumber(t.seller_sequence_number),
    ...
    #[inline(always)] fn push_event(&mut self, event: ExchangeEvent) { if self.event_count < MAX_EVENTS_PER_COMMAND { self.last_events[self.event_count] = Some(event); self.event_count += 1; } }
    #[inline(always)] pub fn events(&self) -> &[Option<ExchangeEvent>] { &self.last_events[..self.event_count] }
    #[inline(always)] pub fn cancel_order(&mut self, instrument_id: u16, account_id: u32, client_order_id: ClientOrderId) -> bool { if !self.instruments.contains(instrument_id) { return false; } self.books[instrument_id as usize].cancel_order(account_id, client_order_id, &mut self.pool) }
    #[inline(always)] pub fn remove_order_by_id(&mut self, instrument_id: u16, account_id: u32, client_order_id: ClientOrderId) -> bool { self.cancel_order(instrument_id, account_id, client_order_id) }
    ...
    fn match_buy(pool: &mut OrderPool, book: &mut OrderBook, trades: &mut [Trade; MAX_TRADES_PER_MATCH], trade_count: &mut usize, instrument_id: u16, incoming_idx: u32) {
        ...
        while pool.data[incoming_idx as usize].remaining > 0 {
            let Some(best_ask) = book.best_ask() else { break }; if incoming_price < best_ask { break; }
            let Some(ask_idx) = book.best_ask_head() else { break }; let ask_price = pool.data[ask_idx as usize].price;
            let fill_qty = pool.data[incoming_idx as usize].remaining.min(pool.data[ask_idx as usize].remaining);
            pool.data[incoming_idx as usize].remaining -= fill_qty; pool.data[ask_idx as usize].remaining -= fill_qty;
            if let Some(level) = book.asks.get_mut(&ask_price) { level.volume = level.volume.saturating_sub(fill_qty); }
            if *trade_count < MAX_TRADES_PER_MATCH { trades[*trade_count] = Trade { buyer: pool.data[incoming_idx as usize].account_id, seller: pool.data[ask_idx as usize].account_id, price: ask_price, qty: fill_qty, instrument_id, buyer_exchange_order_id: pool.data[incoming_idx as usize].exchange_order_id, seller_exchange_order_id: pool.data[ask_idx as usize].exchange_order_id, buyer_client_order_id: pool.data[incoming_idx as usize].client_order_id, seller_client_order_id: pool.data[ask_idx as usize].client_order_id, buyer_sequence_number: pool.data[incoming_idx as usize].sequence_number, seller_sequence_number: pool.data[ask_idx as usize].sequence_number, timestamp: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_nanos() as u64 }; *trade_count += 1; }
            if pool.data[ask_idx as usize].remaining == 0 { book.remove_order(ask_idx, pool); } if *trade_count >= MAX_TRADES_PER_MATCH { break; }
        }
    }
    ...
    fn match_sell(pool: &mut OrderPool, book: &mut OrderBook, trades: &mut [Trade; MAX_TRADES_PER_MATCH], trade_count: &mut usize, instrument_id: u16, incoming_idx: u32) {
        let incoming_price = pool.data[incoming_idx as usize].price;
        while pool.data[incoming_idx as usize].remaining > 0 {
            let Some(best_bid) = book.best_bid() else { break }; if incoming_price > best_bid { break; }
            let Some(bid_idx) = book.best_bid_head() else { break }; let bid_price = pool.data[bid_idx as usize].price;
            let fill_qty = pool.data[incoming_idx as usize].remaining.min(pool.data[bid_idx as usize].remaining);
            pool.data[incoming_idx as usize].remaining -= fill_qty; pool.data[bid_idx as usize].remaining -= fill_qty;
            if let Some(level) = book.bids.get_mut(&bid_price) { level.volume = level.volume.saturating_sub(fill_qty); }
            if *trade_count < MAX_TRADES_PER_MATCH { trades[*trade_count] = Trade { buyer: pool.data[bid_idx as usize].account_id, seller: pool.data[incoming_idx as usize].account_id, price: bid_price, qty: fill_qty, instrument_id, buyer_exchange_order_id: pool.data[bid_idx as usize].exchange_order_id, seller_exchange_order_id: pool.data[incoming_idx as usize].exchange_order_id, buyer_client_order_id: pool.data[bid_idx as usize].client_order_id, seller_client_order_id: pool.data[incoming_idx as usize].client_order_id, buyer_sequence_number: pool.data[bid_idx as usize].sequence_number, seller_sequence_number: pool.data[incoming_idx as usize].sequence_number, timestamp: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_nanos() as u64 }; *trade_count += 1; }
            if pool.data[bid_idx as usize].remaining == 0 { book.remove_order(bid_idx, pool); } if *trade_count >= MAX_TRADES_PER_MATCH { break; }
        }
    }
}
