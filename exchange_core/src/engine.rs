...
///
/// Both exchange-assigned and client-assigned identities are retained so a
/// downstream ledger can reconcile an execution without guessing which ID
/// namespace a field belongs to.
#[repr(C)]
...
impl MatchingEngine {
    ...
    /// Validate and allocate a packet, assigning the exchange order ID only
    ...
    pub fn accept_order(&mut self, packet: &OrderPacket) -> Result<AcceptedOrder, OrderAcceptError> {
        ...
        let pool_index = self.pool.allocate_from_packet(packet, exchange_order_id);
        Ok(AcceptedOrder {
            pool_index,
            exchange_order_id,
        })
